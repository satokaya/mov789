...
$("#user-request").on("submit",function (event) {	
  ...
  var fd = new FormData();
	...
	for (num in fileId) {
		...
		if ( document.getElementById(fileId[num]) != null)
		{
			var ins = document.getElementById(fileId[num]).files.length;
			for (var x = 0; x < ins; x++) {
				//alert(fileId[num]+"[]"+document.getElementById(fileId[num]).files[x])
				fd.append(fileId[num]+"[]", document.getElementById(fileId[num]).files[x]);
			}
		}
...
$("#nuxsell-sign-up,#nuxsell-login").on("submit",function (event) {	
			...
			if (http.readyState === 4) {  
				...
				{  
					//alert(http.responseText)
			     	var	 res_obj=JSON.parse(http.responseText); 
					//alert(http.responseText);
					///$('.error_text').html(http.responseText);
					$('.error_text').html(res_obj[1]);
					if(res_obj[2]!='' && res_obj[2]!=null && res_obj[2]!=undefined)
					{
						if(res_obj[2].length>0)
...
function sendRequestServer(sort_type,increment,limit_member) {
		console.log("req_server"+page+" increment"+increment+ " sort_type"+sort_type+ " limit_member"+limit_member );
...
var noscroll = document.getElementById("noscroll").value;
if(noscroll==1){}else
{
